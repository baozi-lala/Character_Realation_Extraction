...
class BaseModel(nn.Module):
    ...
    def __init__(self, params, pembeds, loss_weight, sizes, maps, lab2ign):
        ...
        self.ner_emb = EmbedLayer(num_embeddings=sizes['type_size'],
                                  embedding_dim=params['type_dim'], dropout=0.0, ignore=0)  # 词实体类型

        self.max_entity_cnt = 42 + 1
        self.coref_pos_emb = EmbedLayer(num_embeddings=self.max_entity_cnt, embedding_dim=params['coref_dim'],
                                        dropout=0.0, ignore=0)

        if params['finaldist']:  # 考虑实体距离的方向关系
        ...
        if not params['loss_weight'] or loss_weight is None:
            print("not load loss_weight")
            self.loss = nn.BCEWithLogitsLoss(reduction='none')
        else:
            loss_weight = torch.from_numpy(loss_weight).to(self.device)
            self.loss = nn.BCEWithLogitsLoss(reduction='none', pos_weight=loss_weight)
    ...
    @staticmethod
    def merge_mentions_old(info, mentions):
        """
    ...
    def merge_mentions(info, mentions, type="mean"):
        ...
        for i in range(index_f.shape[0]):
            entity = pool(mentions, index_f[i, :].unsqueeze(-1), type=type)  # mask 中1是不要的
